...
from app.routers import auth, test, prediction, resume, jobs, users, stats, progress, career_profile
...
def create_tables():
    ...
    from app.models import user_progress  # noqa: ensure model registered
    from app.models import email_verification  # noqa: ensure model registered
    from app.models import password_reset  # noqa: ensure model registered
    Base.metadata.create_all(bind=engine)

    # Idempotent migratsiya: eski users jadvaliga is_verified ustunini qo'shish
    # va mavjud foydalanuvchilarni grandfather qilish (is_verified=true).
...
def smart_chat_response(message: str, career_context: Optional[str]) -> str:
    ...
    if any(w in msg_lower for w in ["salom", "assalomu", "hello", "hi", "hey", "xayr", "rahmat"]):
        return ("Assalomu alaykum! Men Kasbim yordamchisiman. "
    ...
    try:
        ...
        if api_key:
            try:
                ...
                system_prompt = (
                    "Siz Kasbim tizimining yordamchi chatbotiSiz. "
